...
def get_gender_from_given_name(ungendered_imdbs):
    ...
    while(start<len(names_list)):
        ...
        name_string.strip("&")
        url = 'https://api.genderize.io?'+name_string
        r = requests.get(url)
        gendered_names = r.json()
...
print()
print("Genderize.io's gender attribution was only accepted when its probability was at least 90%. Otherwise it was counted towards 'unsure'.  You can find an overview about all crew members and their gender in the file crew_gender_"+film_title+".csv.")
